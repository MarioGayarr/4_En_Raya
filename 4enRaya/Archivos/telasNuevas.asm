...
MOVER_DERECHA:
    ...
    LD A, L
    CP POS_MAX1
    JR Z, DIBUJA_DCHA      ; si he llegado al borde derecho me quedo ahí
    JR C, DIBUJA_DCHA      ; si aún no llego simplemente dibujo y retorno
SALTO_FINAL:
    LD L, 0
DIBUJA_DCHA:
    CALL PintaCuadrao1
...
MOVER_IZQUIERDA:
    ...
    LD A, L
    CP POS_MIN1
    JR Z, SALTO_INICIO     ; al tocar el límite izquierdo salto al final
    JR C, SALTO_INICIO
    DEC L
    CALL PintaCuadrao1
    RET      
...
L15:
    LD BC,$FBFE
    IN A,(C)
    BIT 0,A               ; teclas Q/W leídas desde este puerto
    JR NZ,L16
    CALL SOLTAR_TECLA
    LD A,'Q'
    LD (Caracter),A
...
L17:
    LD BC,$FDFE
    IN A,(C)              ; puerto con la tecla F para terminar
    BIT 3,A
    JR NZ,L5
    CALL SOLTAR_TECLA
    LD A,'F'
...
CAMBIAR_USUARIO:
    CALL BORRAR_FICHA1
    CALL Animacion_Caeficha   ; deja caer la ficha al tablero antes de pasar turno
...
ES_ROJOYNolocambio:
    CALL PintaCuadrao1
L21:
    PUSH BC                 ; Guardar BC al inicio del bucle de lectura
    LD BC,$DFFE
    IN A,(C)
    BIT 0,A
    JR NZ,L22
